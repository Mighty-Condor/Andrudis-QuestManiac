...
class QuestManiac implements IPreAkiLoadMod, IPostDBLoadMod
{
                    ...
                    for (const nextCondition of questContent.conditions.AvailableForFinish) 
                    {
                        ...
                        if (nextConditionData._props.type == "Elimination") 
                        {
                            for (const subCondition of nextConditionData._props.counter.conditions) 
                            {
                                ...
                                {
                                    if (subConditionData._props.target == "Savage") 
                                    {
                                        if (subConditionData._props.savageRole != null && subConditionData._props.savageRole[0] == "pmcBot") 
                                        {
                                            subConditionData._props.target = "AnyPmc"
                                            subConditionData._props.savageRole = null
                                        }
                                    }
                                }
                                ...
                                {
                                    subConditionData._props.target = ["factory4_day", "factory4_night", "bigmap", "Interchange",
                                ...
                                {
                                    subConditionData._props.equipmentExclusive = []
                                }
                            }
                        }
                    }
    ...
    private setupTraderUpdateTime(container: DependencyContainer): void 
    {
        ...
        const configServer = container.resolve<ConfigServer>("ConfigServer")
        const traderConfig = configServer.getConfig<ITraderConfig>(ConfigTypes.TRADER)
        const bashkirRefreshConfig: UpdateTime = { traderId: bashkirBaseJson._id, seconds: 3600 }
        const colonelRefreshConfig: UpdateTime = { traderId: colonelBaseJson._id, seconds: 3600 }
        const elderRefreshConfig: UpdateTime = { traderId: elderBaseJson._id, seconds: 3600 }
